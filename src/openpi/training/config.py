...
class DataConfig:
    ...
    # Optional list of augmentation dataset directories (for Bridge / RLDS mixed loading)
...
class RLDSBridgeDataConfig(DataConfigFactory):
    """
    Config for training on BridgeData V2 using the RLDS data loader.
    Mirrors RLDSDroidDataConfig but maps Bridge keys and (optionally) converts
    absolute 7-D actions to deltas for training.
    """

    rlds_data_dir: str | None = None
    # Keep a simple enum placeholder (mirrors DROID's 'action_space'); unused for Bridge.
    action_space: bridge_rlds_dataset.BridgeActionSpace | None = None
    action_sequence_keys: Sequence[str] = ("actions",)
    # Optional per-frame filter file (same format as DROID's):
    # { "<episode_key>": [[start, end), ...], ... }
    filter_dict_path: str | None = None
